...
def Get_blurred_img(img, img_label, model, resize_shape=(224, 224), Gaussian_param = [51, 50], Median_param = 11, blur_type= 'Gaussian', use_cuda = 1):
    ########################
...
if __name__ == '__main__':
    ...
    input_path = '/home/myh/ScoreRANDA/dataset_area50/'
    ...
    for count, imgname in enumerate(range(1000)):
        imgname = f"{imgname:0>4}"+'.JPEG'
        print(count)
        if count > 2:
            break
        ...
        input_image = load_image(input_path + imgname)
        ...
        basic_visualize(
            input_.cpu(),
            (1-upsampled_mask).type(torch.FloatTensor).cpu(),
            save_path=f"./new_imgs/{count:0>4}_IGOS.png",
        )
