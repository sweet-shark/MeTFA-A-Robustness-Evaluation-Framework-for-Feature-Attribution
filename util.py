...
def topmaxPixel(HattMap, thre_num):
    ii = np.unravel_index(np.argsort(HattMap.ravel())
                          [: thre_num], HattMap.shape)
    # print(ii)
...
def save_heatmap(output_path, mask, img, blur_mask=0):
...
def denormalize(tensor):
    ...
    for i in range(3):
        denormalized[0, i] = denormalized[0, i]*stds[i]+means[i]
...
def normalize(tensor):
    ...
    normalized = tensor.clone().detach()
    for i in range(3):
        # print(normalized[0,i,:3,:3])
        normalized[0, i] = (normalized[0, i]-means[i])/stds[i]
